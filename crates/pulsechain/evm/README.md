> **⚠️ PARTIALLY STALE.** Written pre-implementation. Known inaccuracies: sacrifice credits are
> applied via `increment_balances`, **not** `transact_system_call()`; the Ethereum deposit contract
> is **fully destroyed**, not left alive with nil bytecode; and storage slots must be built with
> `EvmStorageSlot::new_changed(...)`, not `new(...)`. Trust `src/fork_state.rs` and the port's
> `CLAUDE.md` over this file.
